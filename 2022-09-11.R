...
#PALMAS
# Filtro del dia 11 de septiembre de 2022
datos_dia <- datos_bici %>% filter(fecha == 20220911)
...
graficar_serie <- function(datos, experimento) {
  ...
  subplot(p1, p2, p3, nrows = 3, shareX = TRUE, titleY = TRUE) %>%
    layout(title = paste("La Serie de Tiempo del Día - Fecha: 20220911 | Exp:", experimento),
...
matriz_cor <- function(datos_dia, experimento) {
  ...
  matriz_sin_anomalias <- crear_matriz(df_sanos, paste("Matriz sin anomalias - Exp:", experimento, "- 11 sep"))
...
# En el tiempo 139 hubo una caida en la temperatura, en la corriente y una subida 
...
# Correlaciones muy buenas y representativas.



# Este dia cumple años mi hermano 🎊🎂
