...
# Filtro del dia 20 de octubre de 2022
datos_dia <- datos_bici %>% filter(fecha == 20221020)
...
graficar_serie1 <- function(datos, experimento) {
  ...
  subplot(p1, p2, p3, nrows = 3, shareX = TRUE, titleY = TRUE) %>%
    layout(title = paste("La Serie de Tiempo del Día - Fecha: 20221020 | Exp:", experimento),
           xaxis = list(title = "Linea de Tiempo (new_time)"),
           hovermode = "x unified")
}
...
matriz_cor <- function(datos_dia, experimento) {
  ...
  matriz_sin_anomalias <- crear_matriz(df_sanos, paste("Matriz sin anomalias - Exp:", experimento, "- 20 oct"))
  
  if(nrow(df_anomalos) >= 1) {
    matriz_con_anomalias <- crear_matriz(df_anomalos, paste("Matriz con anomalias - Exp:", experimento, "- 20 oct"))
