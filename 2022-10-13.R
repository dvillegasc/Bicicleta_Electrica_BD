...
# Filtro del dia 13 de octubre de 2022
datos_dia <- datos_bici %>% filter(fecha == 20221013)
...
graficar_serie1 <- function(datos, experimento) {
  ...
  subplot(p1, p2, p3, nrows = 3, shareX = TRUE, titleY = TRUE) %>%
    layout(title = paste("La Serie de Tiempo del Día - Fecha: 20221013 | Exp:", experimento),
           xaxis = list(title = "Linea de Tiempo (new_time)"),
           hovermode = "x unified")
}
...
matriz_cor <- function(datos_dia, experimento) {
  ...
  matriz_sin_anomalias <- crear_matriz(df_sanos, paste("Matriz sin anomalias - Exp:", experimento, "- 13 oct"))
  
  if(nrow(df_anomalos) >= 1) {
    matriz_con_anomalias <- crear_matriz(df_anomalos, paste("Matriz con anomalias - Exp:", experimento, "- 13 oct"))
