...
# Filtro del dia 9 de octubre de 2022
datos_dia <- datos_bici %>% filter(fecha == 20221009)

# Para saber la ruta y experimentos
...
graficar_serie1 <- function(datos, experimento) {
  ...
  subplot(p1, p2, p3, nrows = 3, shareX = TRUE, titleY = TRUE) %>%
    layout(title = paste("La Serie de Tiempo del Día - Fecha: 20221009 | Exp:", experimento),
...
matriz_cor <- function(datos_dia, experimento) {
  ...
  matriz_sin_anomalias <- crear_matriz(df_sanos, paste("Matriz de Correlacion - Exp:", experimento, "- 9 oct"))
