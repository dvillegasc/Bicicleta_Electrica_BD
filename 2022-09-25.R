...
# Filtro del dia 25 de septiembre de 2022
datos_dia <- datos_bici %>% filter(fecha == 20220925)
...
graficar_serie1 <- function(datos, experimento) {
  ...
  subplot(p1, p2, p3, nrows = 3, shareX = TRUE, titleY = TRUE) %>%
    layout(title = paste("La Serie de Tiempo del Día - Fecha: 20220925 | Exp:", experimento),
...
matriz_cor <- function(datos_dia, experimento) {
  ...
  matriz_sin_anomalias <- crear_matriz(df_sanos, paste("Matriz sin anomalias - Exp:", experimento, "- 25 sep"))
  
  if(nrow(df_anomalos) > 0) {
    matriz_anomalias <- crear_matriz(df_anomalos, paste("Matriz con anomalias - Exp:", experimento, "- 25 sep"))
